...
class SetCriterion(nn.Module):
    ...
    def loss_img_label(self, outputs, targets, *args, **kwargs):
        ...
        if self.add_relation_token:
            ...
            target_attn_label = torch.stack([(t['attention_label']).type(torch.uint8).any(dim=0) for t in targets]).float()
            target_spat_label = torch.stack([(t['spatial_label']).type(torch.uint8).any(dim=0) for t in targets]).float()
            target_cont_label = torch.stack([(t['contact_label']).type(torch.uint8).any(dim=0) for t in targets]).float()
            attn_logits = outputs['x_attn_logits']
            spat_logits = outputs['x_spat_logits']
            cont_logits = outputs['x_cont_logits']
