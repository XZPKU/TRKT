...
def run_train(device='0', layer_to_det=24, tag='refine'):
    ...
    path_to_coco = "/network_space/server127/shared/vidvrd/action-genome"
    ...
    os.system(f"python -m torch.distributed.launch --master_port {port}\
                    ...
                    --cam_thr 0.2 \
                    --add_relation_token \
                    --resume /network_space/server126/shared/xuzhu/CSA/refine/output/raft/checkpoint0009.pth \
                    --eval")
                    # --rel_loss_coeff 1 \
                    # --add_relation_token \
                    # --extend_rel_token \
...
if __name__ == "__main__":
    device = '2,3'#'0,1,2,3,4,5,6,7'
    run_train(device=device, layer_to_det=24, tag='raft')
